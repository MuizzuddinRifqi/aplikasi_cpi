...
<script>
	function getdataKriteria() {
		...
		$.ajax({
			url: "http://localhost/aplikasi_cpi/modul/apikinerja.php",
			type: "POST",
			dataType: "json",
			data: {
				id: id
...
<script>
	function getDataGuru() {
		...
		$.ajax({
			url: "http://localhost/aplikasi_cpi/modul/apiguru.php",
			type: "POST",
			dataType: "json",
			data: {
				id: id
...
<script>
	function insertDataNilai() {
		...
		if (!id_kriteria || !nilai) {
			...
			$.ajax({
				url: "http://localhost/aplikasi_cpi/modul/apiinsertnilai.php",
				type: "POST",
				dataType: "json",
				data: {
				...
				success: function(hasil) {
					if (hasil.message === "success") {
						document.location = "http://localhost/aplikasi_cpi/dashboard.php?module=input_nilai&periode=" + periode + "&guru=" + guru;
...
<script>
	function insertDataKinerja() {
		...
		$.ajax({
			url: "http://localhost/aplikasi_cpi/modul/apiinsertkinerja.php",
			type: "POST",
			dataType: "json",
			data: {
				id_kinerja: id_kinerja,
			...
			success: function(hasil) {
				if (hasil.message = "success") {
					document.location = "http://localhost/aplikasi_cpi/dashboard.php?module=data_nilai";
				} else {
					alert("data ada yang salah");
				}
			}
						...
						<div class="col-xs-12 col-sm-12 col-md-6">
							<div class="card card-secondary">
								...
								<div class="form-group">
									...
									<div class="col-sm-12">
										<input type="text" name="id_kinerja" id="id_kinerja" value="<?php echo $numberKinerja; ?>" class="form-control" placeholder="" required>
									</div>
								</div>
								<div class="form-group">
